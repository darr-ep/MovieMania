  ...
  async function fetchMovies() {
    try {
      for (const favorita of favoritos) {
        const idMovie = favorita.pelicula_id;
...
function renderMovies() {
  if (!sesionActiva) {
    const pendientesContenido = document.getElementById("favoritas__tarjeta");
    pendientesContenido.innerHTML = "Inicia sesión para ver tus películas favoritas";
    return;
  }
